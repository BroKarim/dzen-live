    ...
    <div className="relative min-h-screen w-full overflow-x-hidden transition-colors duration-300">
      ...
      <div className="relative z-10 min-h-screen" style={{ padding: `${profile.padding || 32}px` }}>
        <div className="absolute top-0 left-0 right-0 z-20 px-6 pt-8">
          <ProfileHeaderButtons name={profile.user?.name} username={profile.username} avatarUrl={avatarUrl} bgType={profile.bgType} bgColor={profile.bgColor} bgWallpaper={profile.bgWallpaper} bgImage={profile.bgImage} displayNameStyle={profile.displayNameStyle} />
        </div>
